...
const targetData = computed<any[]>(() =>
  targets.value
    .map(key => props.data?.find((item: any) => item.code === key))
...
const sourceData = computed(
  () =>
    props.data?.filter(
      (item: any) => targets.value.indexOf(item.code) === -1
    ) ?? []
)
...
const search_el_icon = computed(() =>
  focusing.value ? 'el-icon-search' : 'el-icon-plus'
)
